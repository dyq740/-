...
public class GosControlModuleBaseActivity extends GosBaseActivity {
	 ...
	 * ===========================================================
	 */
	// 数据点"gps_jingdu"对应的标识名
	protected static final String KEY_GPS_JINGDU = "gps_jingdu";
	// 数据点"gps_weidu"对应的标识名
	protected static final String KEY_GPS_WEIDU = "gps_weidu";
	// 数据点"gps_speed"对应的标识名
	protected static final String KEY_GPS_SPEED = "gps_speed";
	 ...
	 * ===========================================================
	 */
	// 数据点"gps_jingdu"对应seekbar滚动条补偿值
	protected static final int GPS_JINGDU_OFFSET = 0;
	// 数据点"gps_jingdu"对应数据点增量值
	protected static final int GPS_JINGDU_ADDITION = 0;
	// 数据点"gps_jingdu"对应数据点定义的分辨率
	protected static final int GPS_JINGDU_RATIO = 1;
		
	// 数据点"gps_weidu"对应seekbar滚动条补偿值
	protected static final int GPS_WEIDU_OFFSET = 0;
	// 数据点"gps_weidu"对应数据点增量值
	protected static final int GPS_WEIDU_ADDITION = 0;
	// 数据点"gps_weidu"对应数据点定义的分辨率
	protected static final int GPS_WEIDU_RATIO = 1;
	 ...
	 * ===========================================================
	 */
	// 数据点"gps_jingdu"对应的存储数据
	protected static double data_gps_jingdu;
	// 数据点"gps_weidu"对应的存储数据
	protected static double data_gps_weidu;
	// 数据点"gps_speed"对应的存储数据
	protected static double data_gps_speed;
	//速度转换系数
	protected static double xishu = 000.1852;
	...
	protected void getDataFromReceiveDataMap(ConcurrentHashMap<String, Object> dataMap) {
		...
		if (dataMap.get("data") != null) {
			...
			for (String dataKey : map.keySet()) {
				if (dataKey.equals(KEY_GPS_JINGDU)) {
			
					data_gps_jingdu = (Integer) map.get(dataKey);
				}
				if (dataKey.equals(KEY_GPS_WEIDU)) {
			
					data_gps_weidu = (Integer) map.get(dataKey);
				}
				if (dataKey.equals(KEY_GPS_SPEED)) {
			
					data_gps_speed = (Integer) map.get(dataKey);
				}
			}
		}
